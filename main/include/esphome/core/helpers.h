...
// std::bit_cast from C++20
    ...
    template <typename To, typename From,
              enable_if_t<sizeof(To) == sizeof(From) && is_trivially_copyable<From>::value &&
    ...
    template <> constexpr14 uint8_t byteswap(uint8_t n)
    {
        return n;
    }
    template <> constexpr14 uint16_t byteswap(uint16_t n)
    {
        return __builtin_bswap16(n);
    }
    template <> constexpr14 uint32_t byteswap(uint32_t n)
    {
        return __builtin_bswap32(n);
    }
    template <> constexpr14 uint64_t byteswap(uint64_t n)
    {
        return __builtin_bswap64(n);
    }
    template <> constexpr14 int8_t byteswap(int8_t n)
    {
        return n;
    }
    template <> constexpr14 int16_t byteswap(int16_t n)
    {
        return __builtin_bswap16(n);
    }
    template <> constexpr14 int32_t byteswap(int32_t n)
    {
        return __builtin_bswap32(n);
    }
    template <> constexpr14 int64_t byteswap(int64_t n)
    {
        return __builtin_bswap64(n);
    }
    ...
    {
        return ((static_cast<uint32_t>(byte1) << 16) | (static_cast<uint32_t>(byte2) << 8) |
    ...
    /// Encode a value from its constituent bytes (from most to least significant) in an std::array with length
    /// sizeof(T).
    template <typename T, enable_if_t<std::is_unsigned<T>::value, int> = 0>
    ...
    {
        return (reverse_bits(static_cast<uint8_t>(x & 0xFF)) << 8) |
               reverse_bits(static_cast<uint8_t>((x >> 8) & 0xFF));
    }
    ...
    /// Convert the value to a string (added as extra overload so that to_string() can be used on all stringifiable
    /// types).
    ...
    optional<T> parse_number(const char *str)
    {
        ...
        if (end == str || *end != '\0' || value < std::numeric_limits<T>::min() ||
     ...
     *
     * When \p len is less than \p 2*count, the result is written to the back of \p data (i.e. this function treats \p
     * str as if it were padded with zeros at the front).
     *
     ...
     *
     * Usually the ESPHome main loop runs at 60 Hz, sleeping in between invocations of `loop()` if necessary. When a
     * higher execution frequency is necessary, you can use this class to make the loop run continuously without
     * waiting.
     */
    ...
    template <class T> class ExternalRAMAllocator
    {
      public:
        ...
        enum Flags {
            NONE = 0,
            REFUSE_INTERNAL =
                1 << 0, ///< Refuse falling back to internal memory when external RAM is full or unavailable.
        ...
        T *allocate(size_t n)
        {
            ...
            if (ptr == nullptr && (this->flags_ & Flags::REFUSE_INTERNAL) == 0) {
                ptr = static_cast<T *>(
                    malloc(size)); // NOLINT(cppcoreguidelines-owning-memory,cppcoreguidelines-no-malloc)
            }
     ...
     */
    template <typename T, enable_if_t<!std::is_pointer<T>::value, int> = 0> T id(T value)
    {
        return value;
    }
    /** Helper function to make `id(var)` known from lambdas work in custom components.
